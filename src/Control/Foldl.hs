...
fold1 :: (F.Foldable f) => Fold a b -> f a -> b
fold1 (Fold step nil done) as = done (F.foldl' step nil as)
{-# INLINE fold1 #-}

fold2 :: Fold a b -> [a] -> b
fold2 (Fold step nil done) as = done (foldl' step nil as)
{-# INLINE fold2 #-}

fold3 :: Fold a b -> [a] -> b
fold3 (Fold step nil done) as = done (foldr step' id as nil)
  where
