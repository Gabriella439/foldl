...
{-| @mkFold1 step@ returns a new 'Fold' using just a step function that has the
...
the 'Foldable', the result is 'None' for empty containers.
 -}
mkFold1 :: (a -> a -> a) -> Fold a (Maybe a)
mkFold1 step = Fold step_ Nothing' lazy
  where
